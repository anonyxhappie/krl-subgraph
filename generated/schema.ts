...
export class ScholarshipHolder extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));

    this.set("racerId", Value.fromBigInt(BigInt.zero()));
    this.set("manager", Value.fromBytes(Bytes.empty()));
    this.set("scholar", Value.fromBytes(Bytes.empty()));
  }

  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save ScholarshipHolder entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        "Cannot save ScholarshipHolder entity with non-string ID. " +
          'Considering using .toHex() to convert the "id" to a string.'
      );
      store.set("ScholarshipHolder", id.toString(), this);
    }
  }

  static load(id: string): ScholarshipHolder | null {
    return changetype<ScholarshipHolder | null>(store.get("ScholarshipHolder", id));
  }
  ...
  get racerId(): BigInt {
    let value = this.get("racerId");
    return value!.toBigInt();
  }

  set racerId(value: BigInt) {
    this.set("racerId", Value.fromBigInt(value));
  }
  ...
  get scholar(): Bytes {
    let value = this.get("scholar");
    return value!.toBytes();
  }

  set scholar(value: Bytes) {
    this.set("scholar", Value.fromBytes(value));
  }
}
